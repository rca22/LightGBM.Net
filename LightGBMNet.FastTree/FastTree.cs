...
namespace LightGBMNet.FastTree
{
    ...
    /// </summary>
    public interface IPredictorWithFeatureWeights<TResult> : IHaveFeatureWeights, IPredictorProducing<TResult>
    {
    }

#if barf
    public abstract class FastTreeTrainerBase<TArgs, TPredictor> // : TrainerBase<TPredictor>
    ...
    internal interface ICacheBins
    {
        Tuple<bool, double[]> CalculateBins(int iFeature, int maxBins, int minDocsPerLeaf, Func<Tuple<bool, double[]>> create);
        SingletonFeatureFlock CreateSingletonFlock(int iFeature, int maxBins, int minDocsPerLeaf, Func<SingletonFeatureFlock> create);
    }

    internal class CachedBins
    {
        public bool HasMissing;
        public double[] UpperBounds;
        public SingletonFeatureFlock FeatureFlock;
    }

    public sealed class CacheBins : ICacheBins
    {
        private Dictionary<Tuple<int, int, int>, CachedBins> _cache;

        public CacheBins()
        {
            _cache = new Dictionary<Tuple<int, int, int>, CachedBins>();
        }

        Tuple<bool, double[]> ICacheBins.CalculateBins(int iFeature, int maxBins, int minDocsPerLeaf, Func<Tuple<bool, double[]>> create)
        {
            lock (_cache)
            {
                var key = new Tuple<int, int, int>(iFeature, maxBins, minDocsPerLeaf);
                CachedBins prev = null;
                if (_cache.TryGetValue(key, out prev))
                    return new Tuple<bool, double[]>(prev.HasMissing, prev.UpperBounds);
                else
                {
                    var rslt = create();
                    _cache[key] = new CachedBins { HasMissing = rslt.Item1, UpperBounds = rslt.Item2 };
                    return rslt;
                }
            }
        }

        SingletonFeatureFlock ICacheBins.CreateSingletonFlock(int iFeature, int maxBins, int minDocsPerLeaf, Func<SingletonFeatureFlock> create)
        {
            lock (_cache)
            {
                var key = new Tuple<int, int, int>(iFeature, maxBins, minDocsPerLeaf);
                ...
                if (_cache.TryGetValue(key, out prev))
                {
                    if (prev.FeatureFlock == null)
                        prev.FeatureFlock = create();
                    return prev.FeatureFlock;
                }
                else
                    return create();
            }
    ...
    public interface IDataViewWithColumnOverride : IDataView
    {
        int ColumnIndex { get; set; }
        float[] ColumnData { get; set; }
    }

    public class DataViewWithColumnOverride : IDataViewWithColumnOverride
    {
        private readonly IDataView _view;

        public float[] ColumnData { get; set; }
        public int ColumnIndex { get; set; }

        public DataViewWithColumnOverride(IDataView view)
        {
            _view = view;
        }

        private class RowCursorColumnOverride : IRowCursor
        {
            private readonly IRowCursor _cursor;
            private readonly float[] _values;
            private readonly int _col;

            public RowCursorColumnOverride(IRowCursor cursor, float[] values, int col)
            {
                _cursor = cursor;
                _values = values;
                _col = col;
            ...
            public ISchema Schema => _cursor.Schema;

            public long Position => _cursor.Position;

            public long Batch => _cursor.Batch;
            ...
            public ValueGetter<TValue> GetGetter<TValue>(int col)
            {
                if (col == _col)
                {
                    ValueGetter<float> getter = (ref float value) => value = _values[_cursor.Position];
                    return (getter as Object) as ValueGetter<TValue>;
                }
                else
            ...
            {
                return _cursor.GetIdGetter();
            }

            public ICursor GetRootCursor()
            {
                return _cursor.GetRootCursor();
            }
        ...
        {
            var cursor = _view.GetRowCursor(needCol, rand);
            return needCol(ColumnIndex) ? new RowCursorColumnOverride(cursor, ColumnData, ColumnIndex) : cursor;
        }
    ...
    public sealed class TransposeDataViewWithColumnOverrideAndCache : DataViewWithColumnOverride, ITransposeDataView, ICacheBins
    {
        private readonly ITransposeDataView _view;
        private readonly ICacheBins _cache;

        public TransposeDataViewWithColumnOverrideAndCache(ITransposeDataView view, CacheBins cache) : base(view)
        {
            _view = view;
            _cache = cache;
        }

        private class SlotCursorColumnOverride : ISlotCursor
        {
            private readonly ISlotCursor _cursor;
            private readonly float[] _values;

            public SlotCursorColumnOverride(ISlotCursor cursor, float[] values)
            {
                _cursor = cursor;
                _values = values;
            }
            ...
            {
                ValueGetter<VBuffer<float>> getter = (ref VBuffer<float> values) => values = new VBuffer<float>(_values.Length, _values);
                return (getter as Object) as ValueGetter<VBuffer<TValue>>;
            }
        ...
        public ITransposeSchema TransposeSchema => _view.TransposeSchema;

        public ISlotCursor GetSlotCursor(int col)
        {
            var cursor = _view.GetSlotCursor(col);
            return (col == ColumnIndex) ? new SlotCursorColumnOverride(cursor, ColumnData) : cursor;
        }

        Tuple<bool, double[]> ICacheBins.CalculateBins(int iFeature, int maxBins, int minDocsPerLeaf, Func<Tuple<bool, double[]>> create)
        {
            return _cache.CalculateBins(iFeature, maxBins, minDocsPerLeaf, create);
        }

        SingletonFeatureFlock ICacheBins.CreateSingletonFlock(int iFeature, int maxBins, int minDocsPerLeaf, Func<SingletonFeatureFlock> create)
        {
            return _cache.CreateSingletonFlock(iFeature, maxBins, minDocsPerLeaf, create);
        }
    }
    ...
    internal abstract class DataConverter
    {
            ...
            IParallelTraining parallelTraining, int[] categoricalFeatureIndices, bool categoricalSplit)
        {
            ...
            using (var ch = host.Start("CreateConverter"))
            {
                if (!diskTranspose)
                    ...
                    conv = new DiskImpl(data, host, maxBins, maxLabel, minDocsPerLeaf, kind, parallelTraining, categoricalFeatureIndices, categoricalSplit);
                ch.Done();
            }
            return conv;
        }
        ...
        private sealed class DiskImpl : DataConverter
        {
            ...
            public DiskImpl(RoleMappedData data, IHost host, int maxBins, Float maxLabel, int minDocsPerLeaf, PredictionKind kind,
                IParallelTraining parallelTraining, int[] categoricalFeatureIndices, bool categoricalSplit)
                : base(data, host, null, maxLabel, kind, categoricalFeatureIndices, categoricalSplit)
            {
                ...
                _dataset = Construct(data, ref _numExamples, maxBins, minDocsPerLeaf, parallelTraining);
            }
            ...
            {
                _dataset = Construct(data, ref _numExamples, -1, -1, null);
            }
            ...
            private Dataset Construct(RoleMappedData examples, ref int numExamples, int maxBins, int minDocsPerLeaf, IParallelTraining parallelTraining)
            {
                ...
                using (var ch = Host.Start("Conversion"))
                {
                    ...
                    var dataCached = data as ICacheBins;

                    var labelOverride = data as IDataViewWithColumnOverride;
                    if (labelOverride != null)
                        labelOverride.ColumnIndex = examples.Schema.Label.Index;
                                ...
                                using (var cursor = trans.GetSlotCursor(featIdx))
                                {
                                    ...
                                    while (cursor.MoveNext())
                                    {
                                        ...
                                        if (BinUpperBounds[iFeature] == null)
                                        {
                                            ...
                                            var numExamplesLocal = numExamples;
                                            Func<Tuple<bool, double[]>> create = () =>
                                             {
                                                 getter(ref temp);
                                                 Host.Assert(temp.Length == numExamplesLocal);

                                                 // Must copy over, as bin calculation is potentially destructive.
                                                 copier(ref temp, ref doubleTemp);

                                                 double[] upperBounds;
                                                 var hasMissingTemp = !CalculateBins(finder, ref doubleTemp, maxBins, minDocsPerLeaf,
                                                     ref distinctValues, ref distinctCounts, out upperBounds);
                                                 return new Tuple<bool, double[]>(hasMissingTemp, upperBounds);
                                             };
                                            var rslt = (dataCached != null) ? dataCached.CalculateBins(iFeature, maxBins, 0, create) : create();
                                            hasMissing = rslt.Item1;
                                            BinUpperBounds[iFeature] = rslt.Item2;
                                        }
                                        else
                                        {
                                            getter(ref temp);
                            ...
                            using (var catCursor = trans.GetSlotCursor(featIdx))
                            {
                                ...
                                else
                                {
                                    for (int i = 0; i < NumFeatures; i++)
                                    {
                                        ...
                                        Func<SingletonFeatureFlock> createFlock = () =>
                                        {
                                            GetFeatureValues(cursor, i, getter, ref temp, ref doubleTemp, copier);
                                            return CreateSingletonFlock(ch, ref doubleTemp, binnedValues, upperBounds);
                                        };
                                        flocks.Add((dataCached != null) ? dataCached.CreateSingletonFlock(iFeature, maxBins, 0, createFlock) : createFlock());
                                    }
                                }
      ...
      //ISingleCanSaveOnnx
    {
        ...
        protected FastTreePredictionWrapper(Ensemble trainedEnsemble, int numFeatures)
            //: base(env, name)
        {
        ...
        protected FastTreePredictionWrapper(BinaryReader reader)
        {
            ...
            TrainedEnsemble = new Ensemble(reader, usingDefaultValues, categoricalSplits);
            MaxSplitFeatIdx = FindMaxFeatureIndex(TrainedEnsemble);
        ...
        public virtual void GetOutput(ref VBuffer<Float> src, ref Float dst)
        {
            ...
            //else
            if(!(src.Length > MaxSplitFeatIdx)) throw new ArgumentException("Feature vector too small");
        ...
        public void GetFeatureWeights(ref VBuffer<Float> weights)
        {
            ...
            Double normFactor = max == 0 ? 1.0 : (1.0 / Math.Sqrt(max));
            var bldr = new BufferBuilder<Float>(FloatAdder.Instance);
