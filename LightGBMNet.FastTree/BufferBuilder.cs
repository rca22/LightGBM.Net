...
namespace LightGBMNet.FastTree
{
    ...
    public sealed class BufferBuilder<T>
    {
        ...
        private void AssertValid()
        {
            ...
            Debug.Assert(_values.Length >= _count);
            Debug.Assert(0 <= _ifeatCur & 0 <= _cfeatCur & _ifeatCur <= _length - _cfeatCur);

            if (_dense)
                Debug.Assert(_count == _length);
            else
            {
                // ResetImpl forces _indices != null and _length > 0.
                Debug.Assert(_indices != null || _length == 0);
                Debug.Assert((_indices?.Length ?? 0) >= _count);

                // If we have no more than InsertThreshold items, we always keep things sorted.
                Debug.Assert(_sorted | _count > InsertThreshold);
            }
#endif
        }
        ...
        public static BufferBuilder<T> CreateDefault()
        {
            ...
            throw new Exception($"Unrecognized type '{typeof(T)}' for default {nameof(BufferBuilder<T>)}");
        }
        ...
        {
            AssertValid();
            Debug.Assert(0 <= ifeat & 0 <= cfeat & ifeat <= _length - cfeat);
            _ifeatCur = ifeat;
            _cfeatCur = cfeat;
            AssertValid();
        }
        ...
        {
            AssertValid();
            Debug.Assert(0 <= index & index < _cfeatCur);
            ...
            Debug.Assert(ivDst < 0 || _indices[ivDst] < index);
            ivDst++;
            Debug.Assert(ivDst == _count || _indices[ivDst] > index);
        ...
        private void SortAndSumDups()
        {
            ...
            Debug.Assert(ivSrc < _count && !_comb.IsDefault(_values[ivSrc]));

            _values[ivDst] = _values[ivSrc];
            _indices[ivDst++] = _indices[ivSrc];
            while (++ivSrc < _count)
            {
                Debug.Assert(ivDst <= ivSrc);
            ...
            Debug.Assert(0 < ivDst & ivDst <= _count);
            _count = ivDst;
            _sorted = true;
            AssertValid();
        ...
        public bool TryGetFeature(int index, out T v)
        {
            ...
            int iv = VBuffer<int>.FindIndexSorted(_indices, 0, _count, ifeat);
            Debug.Assert(iv == 0 || ifeat > _indices[iv - 1]);

            if (iv < _count)
            {
                Debug.Assert(ifeat <= _indices[iv]);
        ...
        private void GetResult(ref T[] values, ref int[] indices, out int count, out int length)
        {
            ...
            {
                Debug.Assert(_count < _length);
                if ((values?.Length ?? 0) < _count)
                    values = new T[_count];
                if ((indices?.Length ?? 0) < _count)
        ...
        {
            if (!(0 <= index && index <= _length - buffer.Length))
